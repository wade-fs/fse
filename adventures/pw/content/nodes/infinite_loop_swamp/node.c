...
int receive_execution(object player, mapping ast) {
    ...
    // 🚀 資料驅動：從自包含目錄載入挑戰預期配置
    string quest_content = read_file("/content/nodes/infinite_loop_swamp/challenges/infinite_loop_termination.yaml");
    if (!quest_content) {
    ...
    int has_factor = load_object("/runtime/services/factor_service.c")->factor_discovered(player, "loop_termination");
    ...
    // 觸發因素探索解鎖
    load_object("/runtime/services/factor_service.c")->discover_factor(player, "loop_termination");

...
int receive_execution(object player, mapping ast) {
    ...
    // 🚀 資料驅動：從自包含目錄載入挑戰預期配置
    string quest_content = read_file("/content/nodes/counter_valley/challenges/count_to_100.yaml");
    if (!quest_content) {
    ...
    if (ast["type"] == expected["type"] && 
        ...
        // 觸發任務完成事件
        load_object("/runtime/services/progress_service.c")->complete_quest(player, "count_to_100");
        return 1;
    }

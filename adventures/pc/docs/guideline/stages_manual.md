...
5. **即死性危害與試錯學習（Instakill vs. FSE Learn Loop）**：
   * 某些終極掠食者（如霸王龍）的 `hp_change` 被設定為 `-100`（即死）。這代表在當前大氣與物理條件下，「正面暴露於該掠食者感知範圍」是一個絕對無法承受的物理後果。
   * 為了與 FSE 的「失敗是學習的必經之路（Fail to Learn）」配合，即死雖然會導致玩家死亡，但系統仍會為其扣減並記錄 `caught_by_predator` 的 **`Confusion` (困惑狀態)**。
   * 玩家死亡後會觸發資料驅動的階段墓地復活機制，在當前階段起點（如 `cretaceous_shore`）重新出生，帶著這份「困惑」回想環境細節，促使玩家轉而尋求徹底規避偵測（例如透過靜止偽裝 `freeze` 解鎖 `tyrex_senses`），而不是採取強行試錯扣血的蠻幹路徑。

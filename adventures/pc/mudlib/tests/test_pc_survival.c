...
int run_all_tests() {
    ...
    if (!canyon || !player) {
        write(HIR "❌ 測試失敗: 無法初始化玩家或乾燥峽谷房間物件。\n" NOR);
        return 1;
    }
    ...
    // ----------------------------------------------------
    // 🧪 測試 1：在乾燥峽谷嘗試奔跑以測試缺氧機制
    // ----------------------------------------------------
    write(CYN "🧪 [測試 1] 在乾燥峽谷嘗試奔跑以測試缺氧機制...\n" NOR);
    player->force_me("run");
    if (!player->has_factor("oxygen_scarcity")) {
        write(HIR "❌ 測試 1 失敗: 奔跑後未獲得 oxygen_scarcity factor！\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 奔跑缺氧倒地死亡並領悟 oxygen_scarcity。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 2：觸碰地表岩石測試高溫灼傷機制
    // ----------------------------------------------------
    write(CYN "🧪 [測試 2] 觸碰地表岩石測試高溫灼傷機制...\n" NOR);
    player->force_me("touch ground");
    if (!player->has_factor("heat_regulation")) {
        write(HIR "❌ 測試 2 失敗: 觸碰地表岩石後未獲得 heat_regulation factor！\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 高溫灼傷死亡並領悟 heat_regulation。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 3：檢驗背光陰影出口 (shade) 是否解鎖
    // ----------------------------------------------------
    write(CYN "🧪 [測試 3] 檢驗背光陰影出口 (shade) 是否解鎖...\n" NOR);
    mapping exits = canyon->query_exits(player);
    if (undefinedp(exits["shade"])) {
        write(HIR "❌ 測試 3 失敗: 具備 heat_regulation 後，shade 出口依然未 Reveal！\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 成功 Reveal 背光陰影出口。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 4：前往背光陰影 (go shade)
    // ----------------------------------------------------
    write(CYN "🧪 [測試 4] 前往背光陰影 (go shade)...\n" NOR);
    ...
    if (!current_room || strsrch(base_name(current_room), "triassic_shade") == -1) {
        write(HIR "❌ 測試 4 失敗: 玩家移動失敗或未進入背光陰影房間。\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 成功移動至背光洞窟陰影。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 5：在陰影下休息 (rest) 撐過第零天
    // ----------------------------------------------------
    write(CYN "🧪 [測試 5] 在陰影下休息 (rest) 撐過第零天...\n" NOR);
    ...
    if (pm->query_current_stage(player, "main") != "first_night") {
        write(HIR "❌ 測試 5 失敗: 休息後未晉升至 first_night 階段！\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 成功完成 survive_day_zero並晉升至 first_night。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 6：前往史前河床 (go riverbed)
    // ----------------------------------------------------
    write(CYN "🧪 [測試 6] 前往史前河床 (go riverbed)...\n" NOR);
    ...
    if (!current_room || strsrch(base_name(current_room), "triassic_riverbed") == -1) {
        write(HIR "❌ 測試 6 失敗: 未能進入史前河床房間。\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 成功移動至史前河床。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 7：嘗試舔水窪中的水 (lick water)
    // ----------------------------------------------------
    write(CYN "🧪 [測試 7] 嘗試舔水窪中的水 (lick water)...\n" NOR);
    player->force_me("lick water");
    if (!player->has_factor("water_boiling")) {
        write(HIR "❌ 測試 7 失敗: 嚐試水質後未解鎖 water_boiling factor！\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 嚐試水質成功領悟 water_boiling。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 8：前往三疊紀荒原 (go plains)
    // ----------------------------------------------------
    write(CYN "🧪 [測試 8] 前往三疊紀荒原 (go plains)...\n" NOR);
    player->force_me("go plains");
    current_room = environment(player);
    if (!current_room || strsrch(base_name(current_room), "triassic_plains") == -1) {
        write(HIR "❌ 測試 8 失敗: 未能進入三疊紀荒原。\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 成功移動至三疊紀荒原。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 9：專注荒原地面 (focus ground)
    // ----------------------------------------------------
    write(CYN "🧪 [測試 9] 專注荒原地面 (focus ground)...\n" NOR);
    player->force_me("focus ground");
    if (!player->query_temp("found_roots")) {
        write(HIR "❌ 測試 9 失敗: focus ground 之後未標記 found_roots temp 變數！\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 成功在荒原發現乾燥根部。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 10：使用 roots 和 branches 摩擦引火並晉升
    // ----------------------------------------------------
    write(CYN "🧪 [測試 10] 使用 roots 和 branches 摩擦引火...\n" NOR);
    player->force_me("rub branches roots");
    if (!player->has_factor("thermodynamics")) {
        write(HIR "❌ 測試 10 失敗: 具備材料後 rub 依然未獲得 thermodynamics factor！\n" NOR);
        return 1;
    }
    if (pm->query_current_stage(player, "main") != "survivor") {
        write(HIR "❌ 測試 10 失敗: 生火後未晉升至 survivor 階段！\n" NOR);
        return 1;
    }
    write(HIG "  ✓ 通過: 摩擦引火領悟成功，解鎖 thermodynamics 並晉升至 survivor 階段。\n" NOR);

    // ----------------------------------------------------
    // 🧪 測試 11：移動前往巨型蕨類森林 (go forest)
    // ----------------------------------------------------
    write(CYN "🧪 [測試 11] 移動前往巨型蕨類森林 (go forest)...\n" NOR);
    ...
    move_object(player, load_object("/rooms/triassic_plains/room"));

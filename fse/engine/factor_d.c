// /fse/engine/factor_d.c
// FSE (Flow State Engine) 通用核心因素管理器 (Factor Manager)
...
// 從 YAML 載入特定因素定義 (通用載入，fid 對應 /mapping/factors/{fid}.yaml)
mapping load_factor_data(string fid) {
    string path = sprintf("/mapping/factors/%s.yaml", fid);
...
int discover_factor(object player, string fid) {
    ...
    event_bus = load_object("/fse/engine/event_d.c");
    if (event_bus) {

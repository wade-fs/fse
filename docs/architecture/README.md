...
```
       ┌────────────────────────┐
       │   Adventure 故事層     │ (尋仙問道 SO、編程世界 PW 等)
       └───────────┬────────────┘
                   │ 依賴於
                   ▼
       ┌────────────────────────┐
       │ Reality Package 規律包 │ (客觀物理律、精神因果律)
       └───────────┬────────────┘
                   │ 依賴於
                   ▼
       ┌────────────────────────┐
       │   Knowledge 知識樹     │ (等價交換、心氣共鳴)
       └───────────┬────────────┘
                   │ 依賴於
                   ▼
       ┌────────────────────────┐
       │  Runtime Services      │ (event_bus, factor_service)
       └───────────┬────────────┘
                   │ 依賴於
                   ▼
       ┌────────────────────────┐
       │     Core Runtime       │ (虛擬物件編譯、虛擬機基底)
       └────────────────────────┘
```
